"""
Async client class for the War Track Dashboard API.
"""
...
class AsyncClient:
    """
    Async client for interacting with the War Track Dashboard API.

    Example:
        ```python
        import asyncio
        from uruwat import AsyncClient, Country, EquipmentType
    ...
    async def get_equipments(
        ...
        data = await self._request("POST", f"/api/stats/equipments/{country.value}", json=json_body)
    ...
    async def get_total_equipments(
        self,
        ...
        json_body = request_body if request_body else None
        data = await self._request("POST", "/api/stats/equipments", json=json_body)
        if not isinstance(data, list):
            raise WarTrackAPIError(f"Expected list, got {type(data).__name__}")
        return [AllEquipment(**item) for item in data if isinstance(item, dict)]

    async def get_equipment_types(self) -> list[dict[str, str]]:
        """
        ...
        """
        data = await self._request("GET", "/api/stats/equipment-types")
    ...
    async def get_systems(
        ...
        json_body = request_body if request_body else None
        data = await self._request("POST", f"/api/stats/systems/{country.value}", json=json_body)
        if not isinstance(data, list):
            raise WarTrackAPIError(f"Expected list, got {type(data).__name__}")
    ...
    async def get_total_systems(
        ...
        json_body = request_body if request_body else None
        data = await self._request("POST", "/api/stats/systems", json=json_body)
        if not isinstance(data, list):
            raise WarTrackAPIError(f"Expected list, got {type(data).__name__}")
        return [AllSystem(**item) for item in data if isinstance(item, dict)]

    async def get_system_types(self) -> list[dict[str, str]]:
        """
        ...
        """
        data = await self._request("GET", "/api/stats/system-types")
        return data if isinstance(data, list) else []

    async def import_equipments(self) -> dict[str, str]:
        """
        ...
        Returns:
            Response message
        """
        data = await self._request("POST", "/api/import/equipments")
        if not isinstance(data, dict):
            raise WarTrackAPIError(f"Expected dict, got {type(data).__name__}")
        return data

    async def import_all_equipments(self) -> dict[str, str]:
        """
        ...
        Returns:
            Response message
        """
        data = await self._request("POST", "/api/import/all-equipments")
        if not isinstance(data, dict):
            raise WarTrackAPIError(f"Expected dict, got {type(data).__name__}")
        return data

    async def import_systems(self) -> dict[str, str]:
        """
        ...
        Returns:
            Response message
        """
        data = await self._request("POST", "/api/import/systems")
        if not isinstance(data, dict):
            raise WarTrackAPIError(f"Expected dict, got {type(data).__name__}")
        return data

    async def import_all_systems(self) -> dict[str, str]:
        """
        ...
        Returns:
            Response message
        """
        data = await self._request("POST", "/api/import/all-systems")
        if not isinstance(data, dict):
            raise WarTrackAPIError(f"Expected dict, got {type(data).__name__}")
        return data
    ...
    async def import_all(self) -> dict[str, str]:
        ...
        Returns:
            Response message
        """
        data = await self._request("POST", "/api/import/all")
        if not isinstance(data, dict):
            raise WarTrackAPIError(f"Expected dict, got {type(data).__name__}")
        return data
